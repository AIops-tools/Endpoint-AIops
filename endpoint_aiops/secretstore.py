"""Encrypted secret storage for endpoint-aiops.

Secrets (Endpoint API keys) are NEVER written to disk in plaintext. They
live in ``~/.endpoint-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``ENDPOINT_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "endpoint-aiops"
CONFIG_DIR = Path.home() / ".endpoint-aiops"
MASTER_PASSWORD_ENV = "ENDPOINT_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``ENDPOINT_<TARGET>_PASSWORD``; ``prefix='ENDPOINT_'`` and
