...
Rules are loaded from ``~/.endpoint-aiops/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("endpoint-aiops.policy")
        ...
        if not tiers:
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        f"No rules file at {self._path} — high/critical operations require a named approver (ENDPOINT_AUDIT_APPROVED_BY) by default. Author rules.yaml "
