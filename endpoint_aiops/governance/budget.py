...
2. **Runaway breaker (on by default, disable with ``ENDPOINT_RUNAWAY_MAX=0``).**
   The same ``(tool, params)`` called more than ``ENDPOINT_RUNAWAY_MAX`` times
   within ``ENDPOINT_RUNAWAY_WINDOW_SEC`` trips a short cooldown. Catches a tight
