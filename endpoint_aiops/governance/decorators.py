...
Responsibilities:
  ...
  3. Post-log: write audit record to ``~/.endpoint-aiops/audit.db``
...
from endpoint_aiops.governance.audit import detect_agent, get_engine
from endpoint_aiops.governance.budget import BudgetExceeded, get_budget
from endpoint_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from endpoint_aiops.governance.patterns import PatternMatch, get_pattern_engine
from endpoint_aiops.governance.policy import PolicyResult, get_policy_engine
from endpoint_aiops.governance.readonly import is_read_only, read_only_denial
from endpoint_aiops.governance.sanitize import sanitize

_log = logging.getLogger("endpoint-aiops.decorators")
    ...
    *,
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.endpoint-aiops/undo.db and the
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`endpoint_aiops.governance.outcome`). The change may be live, so an
    ...
    if not descriptor:
        return
    try:
        from endpoint_aiops.governance.undo import get_undo_store
...
def _infer_skill(func: Any) -> str:
    ...
    ``endpoint_aiops.ops.jobs`` → ``endpoint-aiops``
    ``mcp_server.server`` → ``endpoint-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("endpoint_aiops") or module.startswith("mcp_server"):
        return "endpoint-aiops"
    return "unknown"
