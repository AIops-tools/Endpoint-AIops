...
Thin httpx wrapper with per-target session reuse and static Bearer auth:
  ...
  * The management server issues a long-lived **API key** (created in its web
    UI). Every request carries ``Authorization: Bearer <api_key>`` — there is
    no token-exchange handshake (unlike OAuth2 systems), so the key is sent
    directly.
...
def _teaching_message(status: int, path: str, body: str) -> str:
    ...
    if status in (401, 403):
        return (
            ...
            f"Check the API key (created in the management server UI) and "
