"""``endpoint-aiops secret`` — manage the encrypted credential store.

Secrets (Endpoint API keys) are stored in ``~/.endpoint-aiops/secrets.enc``
...
from endpoint_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: endpoint-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update ENDPOINT_AIOPS_MASTER_PASSWORD.[/]")
