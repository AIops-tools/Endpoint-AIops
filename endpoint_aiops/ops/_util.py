...
from endpoint_aiops.governance import opt_str, sanitize
...
def envelope(rows: list, limit: int) -> dict:
    """Wrap a capped list so a truncated result announces itself.

    A bare list cannot say "there is more" — the consumer has to infer it from
    the length happening to equal the cap, and a smaller local model faced with
    a long result tends to report either that nothing came back or that it saw
    everything. Every capped list in this package returns this one shape::

        {"items": [...], "returned": 25, "limit": 25, "truncated": true}

    ``truncated`` is measured against the full list, never guessed from a length
    coincidence.
    """
    capped = max(1, int(limit))
    items = list(rows[:capped])
    return {
        "items": items,
        "returned": len(items),
        "limit": capped,
...
def s(value: Any, limit: int = 128) -> str:
    ...
    Folds absence into ``""``. Use only for values that are always present;
    for an optional API field use :func:`opt`, which keeps absence visible.
    """
    return sanitize(str(value if value is not None else ""), limit)


def opt(value: Any, limit: int = 128) -> str | None:
    """Sanitize an optional value, preserving absence as ``None``.

    The management server omits fields it has no value for. Folding that into
    ``""`` (as :func:`s` does) tells a consumer "the field exists and is empty",
    which is a different fact — and one a smaller local model will happily
    invent a reason for. ``None`` serialises to JSON ``null``, so "the server
    never reported this" survives all the way to the agent.
    """
