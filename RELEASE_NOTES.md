# Release notes — endpoint-aiops 0.6.0
...
- **A real IGEL UMS auth strategy** — `POST /umsapi/v3/login` (HTTP Basic → JSESSIONID cookie), cached per connection; `doctor` distinguishes a wrong scheme from wrong credentials. Modelled from IGEL's IMI docs and NOT yet verified against an appliance (see docs/VERIFICATION.md).
- The IGEL dialect targets `/umsapi/v3` on 8443 (was `/api/v2.0` on 443 — both prefix and port were wrong).
- **New `scheme:`** (default https) — a plain-HTTP endpoint behind a reverse proxy was unreachable.
