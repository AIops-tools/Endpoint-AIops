...
from endpoint_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_governed_write_is_refused_in_read_only(read_only):
    ...
    def wipe_fleet(target: str = "lab") -> dict:
        calls.append(target)
        return {"wiped": True}
...
def test_medium_risk_write_is_also_refused(read_only):
    """Only 'low' is a read — medium (e.g. endpoint_reboot) must be refused too."""

    @governed_tool(risk_level="medium")
    def reboot_thing(target: str = "lab") -> dict:
...
@pytest.mark.unit
def test_real_write_tool_is_refused_in_read_only(read_only):
    """The actual remediation tool, not just a synthetic one."""
    from mcp_server.tools import remediation as gov

    with pytest.raises(PolicyDenied):
        gov.endpoint_reboot(endpoint_id="tc01")
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "endpoint_assign_profile" in dropped and "endpoint_reboot" in dropped
        assert "endpoint_list" not in dropped and "overview" not in dropped
        ...
        assert "endpoint_list" in remaining, "reads must still be exposed"
        assert "login_storm_analysis" in remaining
    finally:
        registry.clear()
