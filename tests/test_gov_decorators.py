...
``endpoint_aiops.governance.*`` and binds state to an isolated home via
``ENDPOINT_AIOPS_HOME``. Every governed function under test is SYNTHETIC and
...
import endpoint_aiops.governance.budget as budget_mod
import endpoint_aiops.governance.decorators as dec_mod
...
from endpoint_aiops.governance.budget import BudgetExceeded
...
def dec_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ENDPOINT_AUDIT_APPROVED_BY", "pytest")
    monkeypatch.delenv("ENDPOINT_POLICY_DISABLED", raising=False)
    monkeypatch.delenv("ENDPOINT_MAX_TOOL_CALLS", raising=False)
    monkeypatch.delenv("ENDPOINT_MAX_TOOL_SECONDS", raising=False)
    _reset()
    yield tmp_path
    _reset()
...
def test_budget_exceeded_propagates_and_audits(dec_home, monkeypatch):
    monkeypatch.setenv("ENDPOINT_MAX_TOOL_CALLS", "0")
...
def test_policy_disabled_marks_status_bypassed(dec_home, monkeypatch):
    monkeypatch.setenv("ENDPOINT_POLICY_DISABLED", "1")
    _reset()
