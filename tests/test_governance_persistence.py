...
throwaway home (``ENDPOINT_AIOPS_HOME``) and assert that the rows compliance
...
from endpoint_aiops.governance import PolicyDenied, governed_tool
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ENDPOINT_AUDIT_APPROVED_BY", raising=False)
    monkeypatch.delenv("ENDPOINT_POLICY_DISABLED", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="prod-fleet")
...
def test_high_risk_denied_without_approver_and_denial_is_audited(gov_home):
    ...
    with pytest.raises(PolicyDenied, match="requires 'dual' approval"):
        _drop_widget(name="w2", target="prod-fleet")
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("ENDPOINT_AUDIT_APPROVED_BY", "ops-alice")
    result = _drop_widget(name="w3", target="prod-fleet")
...
def test_operator_rules_file_restores_tier_none_for_high_risk(gov_home):
    ...
    _reset_singletons()
    result = _drop_widget(name="w4", target="prod-fleet")
    assert result["status"] == "dropped"


@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: endpoint_assign_profile
    must capture the prior profile before switching and persist the
    reassign-undo on disk."""
    monkeypatch.setenv("ENDPOINT_AUDIT_APPROVED_BY", "ops-alice")
    ...
    result = gov.endpoint_assign_profile(endpoint_id="tc01", profile_id="prof-new")
    assert result["priorState"]["profileId"] == "prof-old"
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "endpoint_assign_profile"
    assert "prof-old" in undo[0]["undo_params"]

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["endpoint_assign_profile"]
